...
# Monetary / KYC-sensitive verbs
_KYC_ACTIONS = frozenset({"approve", "loan", "transfer"})
...
def evaluate(req: DecisionRequest) -> Tuple[str, List[PolicyViolation]]:
    ...
    # Rule P1: No KYC on approve / loan / transfer → hard reject
    if req.action_type in _KYC_ACTIONS and not kyc_verified:
    ...
    if req.action_type in _KYC_ACTIONS and amount >= CRITICAL_VALUE_THRESHOLD:
    ...
    # Rule P3: Confidence below critical threshold on sensitive actions → hard reject
    if req.action_type in _KYC_ACTIONS and req.confidence < CRITICAL_CONFIDENCE_THRESHOLD:
        violations.append(PolicyViolation(
    ...
    if req.action_type in _KYC_ACTIONS and amount > HIGH_VALUE_THRESHOLD and req.confidence < LOW_CONFIDENCE_THRESHOLD:
        violations.append(PolicyViolation(
    ...
    # Rule P5: Critical value without risk assessment → review only
    if req.action_type in _KYC_ACTIONS and amount >= CRITICAL_VALUE_THRESHOLD:
        risk_assessed = req.input.get("risk_score") or req.input.get("risk_assessed")
        if not risk_assessed:
            violations.append(PolicyViolation(
                rule="P5_CRITICAL_VALUE_NO_RISK_ASSESSMENT",
                reason=f"₹{amount} approval without a risk score in input. Documented risk assessment expected for critical-value transactions.",
                severity="warning",
                block=False,
            ))
